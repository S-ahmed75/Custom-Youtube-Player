...
- (BOOL)loadWithPlayerParams:(NSDictionary *)additionalPlayerParams {
  ...
  if ([playerParams objectForKey:@"playerVars"]) {
    ...
    if (![playerVars objectForKey:@"origin"]) {
        ...
        self.originURL = [NSURL URLWithString:@"https://www.youtube.com"];
         NSLog(@"Value of CARTT = %@", self.originURL );
    } else {
        self.originURL = [NSURL URLWithString: [playerVars objectForKey:@"https://www.youtube.com"]];
        NSLog(@"Value of ATTT; = %@", playerVars );
    }
  } else {
